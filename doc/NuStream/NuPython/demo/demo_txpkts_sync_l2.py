...
for loopnum in range(loop_count):
    ...
    nscmd.config_stream_smac(mac_2)
    nscmd.config_stream_dmac(mac_1)
    ...
    nscmd.config_tx_txtime(1)
    # 0-means wait for a global transmit command
    nscmd.config_tx_isimmediate(0)
    ...
    print ("Port 2   Rx-size65:\t%i" % (nscmd.get_counter_port(port2_idx, nsconst.IDX_PORTCOUNTER_RX_65_127)))
    print ("Port 2   Rx-size128:\t%i" % (nscmd.get_counter_port(port2_idx, nsconst.IDX_PORTCOUNTER_RX_128_255)))
    print ("Port 2   Rx-size256:\t%i" % (nscmd.get_counter_port(port2_idx, nsconst.IDX_PORTCOUNTER_RX_256_511)))
    print ("Port 2   Rx-size512:\t%i" % (nscmd.get_counter_port(port2_idx, nsconst.IDX_PORTCOUNTER_RX_512_1023)))
    print ("Port 2   Rx-size1024:\t%i" % (nscmd.get_counter_port(port2_idx, nsconst.IDX_PORTCOUNTER_RX_1024_1522)))
