...
def ShowInstantVoltage(cidx, sidx):
    ...
    plt.title('NuPOE Slot %d Voltage Counter' %sidx)
    plt.grid(True)
    while isShowChart == True:
        plt.plot(t, nscmd.t451_info_board[cidx].t451_board[sidx].report.counter_inst_volt)
        plt.show()
        plt.pause(0.05)
...
# start connect test
nscmd.t451_start_sample(chassis_idx, slot1_idx)
nscmd.t451_start_connect(chassis_idx, slot1_idx)
