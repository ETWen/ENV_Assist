...
nscmd.set_stream(port1_idx, 2)
nscmd.set_stream(port1_idx, 3)
nscmd.set_stream(port1_idx, 4)
# config rx stream
...
rx_s1 = nscmd.get_counter_stream_rx_pkts(port2_idx, 0)
rx_s2 = nscmd.get_counter_stream_rx_pkts(port2_idx, 1)
print "Port 1   Send:%i" % (nscmd.get_counter_port(port1_idx, nsconst.IDX_PORTCOUNTER_TX_PKT))
print "Port 2 Stream 1 Received:%i" % (rx_s1)
print "Port 2 Stream 2 Received:%i" % (rx_s2)
print "Port 2 Stream 3 Received:%i" % (nscmd.get_counter_stream_rx_pkts(port2_idx, 2))
print "Port 2 Stream 4 Received:%i" % (nscmd.get_counter_stream_rx_pkts(port2_idx, 3))
print "Port 2 Stream 5 Received:%i" % (nscmd.get_counter_stream_rx_pkts(port2_idx, 4))
